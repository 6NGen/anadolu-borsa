...
export default function SuruDegeri({ fiyatlar }: { fiyatlar: HayvanFiyat[] }) {
  ...
  return (
    <div style={{ background: RENKLER.surface, border: `1px solid ${RENKLER.border}`, borderRadius: "12px", padding: "16px", marginTop: "20px" }}>
      <div style={{ fontSize: "10px", color: RENKLER.muted, letterSpacing: "0.1em", marginBottom: "4px", fontWeight: 600 }}>🧮 SÜRÜ DEĞERİ HESAPLAYICI</div>
      <div style={{ fontSize: "9px", color: RENKLER.muted, marginBottom: "14px" }}>Baş sayısını gir; ortalama karkas ağırlığını kendi sürüne göre düzenleyebilirsin.</div>
        ...
        {turler.map((t) => {
          ...
          return (
            <div key={t.norm} style={{ background: RENKLER.bg, border: `1px solid ${RENKLER.border}`, borderRadius: "8px", padding: "11px" }}>
              <label style={{ fontSize: "11px", color: RENKLER.text, display: "flex", alignItems: "center", gap: "5px", marginBottom: "7px", fontWeight: 600 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "5px", marginTop: "7px" }}>
                <span style={{ fontSize: "9px", color: RENKLER.muted }}>karkas</span>
                <input
                  type="number" min={1} inputMode="numeric"
                  value={karkasAyar[t.norm] ?? String(KARKAS_KG[t.norm])}
                  onChange={(e) => setKarkasAyar((p) => ({ ...p, [t.norm]: e.target.value }))}
                  style={{ width: "52px", padding: "3px 6px", fontSize: "11px", background: RENKLER.surface, color: RENKLER.text, border: `1px solid ${RENKLER.border}`, borderRadius: "5px", fontFamily: "var(--font-mono)" }}
                />
                <span style={{ fontSize: "9px", color: RENKLER.muted }}>kg · tahmin</span>
              </div>
              <div style={{ fontSize: "9px", color: RENKLER.muted, marginTop: "6px" }}>~{formatFiyat(Math.round(bf), 0)} TL/baş</div>
              {(adet[t.norm] ?? 0) > 0 && (
                <div style={{ fontSize: "11px", color: renk, fontWeight: 700, marginTop: "3px" }}>= {formatFiyat(Math.round((adet[t.norm] ?? 0) * bf), 0)} TL</div>
              )}
            </div>
          );
        })}
